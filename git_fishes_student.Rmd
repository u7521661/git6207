...
[My GitHub Repository]("D:/git6207")
...
# github can save the code in cloud but is actually not a backup.
```
...
# Write your answer here
#If echo = false, the code block will not appear in the output result
```
...
path <- "D:/git6207/git6207/data/OA_activitydat_20190302_BIOL3207.csv"
...
unique(data$species) #there's no spelling errors.
# Use flextable to render the summary table in a tidy format
summ <- flextable(data %>% summarise(sd = sd(activity),mean = mean(activity)))
summ
```
...
#If the code shows red highlight it represents the original code in github cloud, and the green one represents the local code that has not been uploaded to cloud.
```
...
ggplot(data = data2, aes(x=treatment,y =activity,fill =species, colour = species ))+geom_boxplot()+geom_jitter()
...
ggsave("D:/git6207/git6207/output/figures")
```
